...
- [ ] Confirm final phone/accessory/rental inventory matches what's actually in stock. Right now
      `src/data/phones.ts` and `src/data/rentals.ts` ship with representative sample listings.
- [ ] Update `public/robots.txt` and `public/sitemap.xml` with the confirmed production domain.
- [ ] Replace the four sample testimonials in `src/data/testimonials.ts` with real customer quotes.
      They're placeholder text right now, flagged in a comment at the top of that file.

## Security

- [ ] Set `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `BUSINESS_ORDERS_EMAIL` as Netlify environment
      variables. Never commit these to the repo, see `.env.example`.
...
- [ ] Review the CSP in `netlify.toml` after connecting the final domain and any additional
      third-party scripts. Tighten further if analytics or maps aren't used.
- [ ] Consider adding Cloudflare Turnstile or hCaptcha to the contact, booking and order forms for
      stronger spam protection beyond the honeypot and in-memory rate limiting already in place.
      See `netlify/lib/rate-limit.ts`: it's best-effort per function instance, not distributed.
- [ ] Run `npm audit` and address any high or critical findings before launch. Enable Dependabot
      (or equivalent) on the GitHub repo for ongoing dependency alerts.
- [ ] Confirm HTTPS is enforced and HSTS is active once the custom domain and Netlify SSL cert are
...
- [ ] Confirm Largest Contentful Paint stays under 2.5s on a mid-range Android device over 4G.
      The hero image is the likely LCP element, so keep it compressed once real photography lands.
- [ ] Spot-check bundle sizes with `npm run build`. Routes are already code-split via `React.lazy`,
      and vendor/motion/forms are split into separate chunks (see `vite.config.ts`).
...
- [ ] Verify unique title/meta description render correctly per page (`src/components/Seo.tsx`
      sets these client-side, so confirm they're present in the rendered HTML `<head>` for
      crawlers; consider prerendering or SSR in a later phase if crawler JS execution becomes a
      concern).
